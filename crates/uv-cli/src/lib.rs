...
pub struct PipWheelArgs {
    ...
    /// requirement that's installed. However, including a package in a constraints file will _not_
    /// trigger the installation of that package.
    #[arg(long, short = 'b', alias = "build-constraint", env = EnvVars::UV_BUILD_CONSTRAINT, value_delimiter = ' ', value_parser = parse_maybe_file_path)]
    pub build_constraints: Vec<Maybe<PathBuf>>,
    ...
    /// Ignore package dependencies, instead only building wheels for the packages explicitly listed
    /// on the command line or in the requirements files.
    #[arg(long, overrides_with("deps"))]
    pub no_deps: bool,
    ...
    /// The Python interpreter to use for building wheels.
    ///
    /// By default, wheel building uses the discovered Python interpreter. A path to an alternative
    /// Python can be provided.
    ///
    /// See `uv help python` for details on Python discovery and supported request formats.
    #[arg(
        long,
    ...
    /// The directory into which built wheels should be placed.
    ///
    /// Defaults to `./wheelhouse` in the current working directory.
    #[arg(long, short, default_value = "wheelhouse")]

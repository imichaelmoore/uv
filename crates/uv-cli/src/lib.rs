...
pub enum PipCommand {
    ...
    /// Download packages from an index.
    ///
    /// Downloaded packages are saved to the current directory or to the directory specified by
    /// `--dest`.
    #[command(
...
pub struct PipDownloadArgs {
    ...
    /// Download the editable package based on the provided local file path.
    #[arg(long, short, group = "sources")]
    pub editable: Vec<String>,
    ...
    /// Constraints files are `requirements.txt`-like files that only control the _version_ of a
    /// requirement that's installed. However, including a package in a constraints file will _not_
    /// trigger the download of that package.
    ///
    /// This is equivalent to pip's `--constraint` option.
    #[arg(long, short, alias = "constraint", env = EnvVars::UV_CONSTRAINT, value_delimiter = ' ', value_parser = parse_maybe_file_path)]
    pub constraints: Vec<Maybe<PathBuf>>,
    ...
    ///
    /// Overrides files are `requirements.txt`-like files that force a specific version of a
    /// requirement to be downloaded, regardless of the requirements declared by any constituent
    /// package, and regardless of whether this would be considered an invalid resolution.
    ///
    /// While constraints are _additive_, in that they're combined with the requirements of the
    /// constituent packages, overrides are _absolute_, in that they completely replace the
    ...
    /// The Python interpreter to use for resolving dependencies.
    ///
    /// See `uv help python` for details on Python discovery and supported request formats.
    #[arg(
        long,
    ...
    /// Resolve using the system Python environment.
    ///
    /// By default, uv looks for a Python in a virtual environment or uses one found in the
    /// system `PATH`.
    #[arg(
        long,
        env = EnvVars::UV_SYSTEM_PYTHON,
    ...
    /// Don't install pre-built wheels.
    ///
    /// The given packages will be built and downloaded from source. The resolver will still use
    /// pre-built wheels to extract package metadata, if available.
    ///
    /// Multiple packages may be provided. Disable binaries for all packages with `:all:`. Clear
    /// previously specified packages with `:none:`.
    ...
    ///
    /// WARNING: When specified, uv will select wheels that are compatible with the _target_
    /// platform; as a result, the downloaded distributions may not be compatible with the _current_
    /// platform. Conversely, any distributions that are built from source may be incompatible with
    /// the _target_ platform, as they will be built for the _current_ platform. The
    /// `--python-platform` option is intended for advanced use cases.
    #[arg(long)]
    ...
    ///
    /// By default, packages are downloaded to the current directory.
    #[arg(long, short = 'd')]
